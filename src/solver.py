...
class BaseSolver:
    ...
    def __init__(self, config, paras, mode):
        ...
        if mode == 'train':
            ...
            self.src_config = yaml.load(
                open(config['src']['config'], 'r',encoding='utf-8'), Loader=yaml.FullLoader)
    ...
    def enable_apex(self):
        """
        apex包可使用混合精度 (Automatically Mixed Precision, AMP)的方法，极大加快训练速度;
        1.6 版本之后，PyTorch 出厂自带 AMP;
