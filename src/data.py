...
def collect_audio_batch(batch, audio_transform, mode):
    ...
    input: [(file1,txt1),(file2,txt2),...]
    output: file, audio_feat, audio_len, text(经过)
    collate_fn: merges a list of samples to form a mini-batch of Tensor(s).
       Other knowledge about collate_fn:
...
def load_dataset(n_jobs, use_gpu, pin_memory, ascending, corpus, audio, text):
    """ Prepare dataloader for training/validation
        n_jobs: 流水线的数目
        audio/text: 从config（yaml）文件中读取的配置信息
    """

    # Audio feature extractor [音频特征提取：返回用于提取特征的对象]
    audio_transform, feat_dim = create_transform(audio.copy())
    # Text tokenizer [文本分词：返回用于分词的对象]
    ...
    tr_set, dv_set, tr_loader_bs, dv_loader_bs, mode, data_msg = create_dataset(
        tokenizer, ascending, **corpus)  # **代表将字典以关键字参数传进去
    # Collect function[偏函数partial: 可以理解成固定了某些了传入的参数]
