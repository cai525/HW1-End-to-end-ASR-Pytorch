...
class ASR(nn.Module):
    ...
    def __init__(self, input_size, vocab_size, init_adadelta, ctc_weight, encoder, attention, decoder, emb_drop=0.0):
        ...
        # ctc_weight 即论文 Joint CTC-attention based end-to-end speech recognition using multi-task learning（ICASSP)
        #            中提到的参数λ
    ...
    def forward(self, audio_feature, feature_len, decode_step, tf_rate=0.0, teacher=None,
        ...
        # ========== CTC based decoding ==========
        if self.enable_ctc:
            # CTC的解码端通过一个全连接+softmax的操作获得了归一化的概率输出
...
class Attention(nn.Module):
    ...
    def __init__(self, v_dim, q_dim, mode, dim, num_head, temperature, v_proj,
        ...
        self.proj_q = nn.Linear(q_dim, dim*num_head)    # query project
        self.proj_k = nn.Linear(v_dim, dim*num_head)    # key project
        self.v_proj = v_proj                            # value project
        if v_proj:
