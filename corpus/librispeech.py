...
def read_text(file):
    """Get transcription of target wave file,
       ...
       获得音频的转换文本"""
    src_file = '-'.join(file.split('-')[:-1]) + '.trans.txt'  # 不包括后缀
...
class LibriDataset(Dataset):
    ...
    def __init__(self, path, split, tokenizer, bucket_size, ascending=False):
        ...
        properties:
            self.file_list: (list<str>) 音频文件（.flac）的路径列表
        ...
        file_list = []
        for s in split:
            # ：检索目录下所有.flac格式的文件（注意这里的join是os.path的join,用来拼接路径）
            split_list = list(Path(join(path, s)).rglob("*.flac"))  # 说明：flac是音频格式
